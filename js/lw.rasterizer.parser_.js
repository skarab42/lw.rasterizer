...
function GVal(name, values) {
    if (values[name] !== undefined) {
        var value = values[name].toFixed(2);
        if (value !== lastCommands[name]) {
            lastCommands[name] = value;
            return name + value;
        }
    }
}
...
function rasterize() {
    ...
    function horizontalScan() {
        ...
        for (y = height; y >= 0; y--) {
            ...
            postMessage({ type: 'gcode', data: {
                percent: 100 - Math.round((y / height) * 100),
                text   : text.join('\n')
            }});
        }
    }
    ...
    function diagonalScan() {
        ...
        var n = (settings.imageSize.width * settings.imageSize.height) + 1;
        var total = n;
        ...
        while (n--) {
            ...
            if (out) {
                postMessage({ type: 'gcode', data: {
                    percent: 100 - Math.round((n / total) * 100),
                    text   : text.join('\n')
                }});
    ...
    if (settings.diagonal) {
        var joinPixel = settings.joinPixel;
        settings.joinPixel = false;
        diagonalScan();
        settings.joinPixel = joinPixel;
    }
    else {
        horizontalScan();
    }
